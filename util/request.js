...
const createRequest = (method, url, data, options) => {
    return new Promise((resolve, reject) => {
        // set user-agent from useAgentList
        ...
        if (typeof options.cookie === 'object') {
            const cookie = Object.keys(options.cookie)
                .map(
                    (key) =>
                ...
                .join('; ');
            headers['Cookie'] = cookie + corsCookie;
        ...
        if (options.crypto === 'weapi') {
            let csrfToken = (headers['Cookie'] || '').match(
                /_csrf=([^(;|$)]+)/,
            );
            ...
            const header = {
                ...
                appver: cookie.appver || '8.1.20', // app版本
                versioncode: cookie.versioncode || '140', //版本号
                mobilename: cookie.mobilename, //设备model
                buildver:
                    cookie.buildver || Date.now().toString().substr(0, 10),
        ...
        // get settings to axios and do req
        axios(settings)
            .then((res) => {
                const body = res.data;
				const corsCookie = ' SameSite=None; Secure=true';
                ...
                try {
                    answer.body =
                        options.crypto === 'eapi'
                            ? JSON.parse(encrypt.decrypt(body).toString())
                            : body;
